...
setChaincodePath
LANGUAGE="golang"
pnode_CC_SRC_PATH="github.com/pnode"
dnode_CC_SRC_PATH="github.com/dnode"
rnode_CC_SRC_PATH="github.com/rnode"
...
ORG2_TOKEN=$(curl -s -X POST \
  http://localhost:4000/users \
  -H "content-type: application/x-www-form-urlencoded" \
  -d 'username=Barry2&orgName=Dnode')
echo $ORG2_TOKEN
...
ORG3_TOKEN=$(curl -s -X POST \
  http://localhost:4000/users \
  -H "content-type: application/x-www-form-urlencoded" \
  -d 'username=Tony2&orgName=Rnode')
echo $ORG3_TOKEN
...
## Producer smart contracts deployment + Init
echo "***"
echo "Producer smart contracts deployment + Init"
echo "***"
...
curl -s -X POST \
  ...
  -d "{
	\"peers\": [\"peer0.pnode.example.com\"],
	\"chaincodeName\":\"pcc\",
	\"chaincodePath\":\"$pnode_CC_SRC_PATH\",
...
## Distributor smart contracts deployment + Init
echo "***"
echo "Distributor smart contracts deployment + Init"
echo "***"
...
curl -s -X POST \
  ...
  -d "{
	\"peers\": [\"peer0.pnode.example.com\"],
	\"chaincodeName\":\"dcc\",
	\"chaincodePath\":\"$dnode_CC_SRC_PATH\",
	\"chaincodeType\": \"$LANGUAGE\",
	\"chaincodeVersion\":\"v0\"
}"
echo
...
## Retailor smart contracts deployment + Init 
echo "***"
echo "Retailor smart contracts deployment + Init"
echo "***"
...
curl -s -X POST \
  ...
  -d "{
	\"peers\": [\"peer0.pnode.example.com\"],
	\"chaincodeName\":\"rcc\",
	\"chaincodePath\":\"$rnode_CC_SRC_PATH\",
	\"chaincodeType\": \"$LANGUAGE\",
	\"chaincodeVersion\":\"v0\"
}"
echo
